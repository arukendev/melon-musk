...
@WebServlet("/MyReviewsC")
public class MyReviewsC extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		AuthDAO.loginCheck(request);
		HttpSession hs = request.getSession();
		Auth a =(Auth)hs.getAttribute("account");
		ReviewDAO.getNotice(request);
		ReviewDAO.getMyReviews(request);
		ReviewDAO.paging(1, request);
		request.setAttribute("contentPage", "jsp/review/review_my.jsp");
